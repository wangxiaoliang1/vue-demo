  ...
  <div class="page-style">
    ...
    >
      <Form
        ...
        :label-colon="true"
      >
        <FormItem :label="'Rule' + $t('name')" prop="name">
          <Input v-model="submitData.name" :placeholder="$t('pleaseEnter') + 'Rule' + $t('name')" />
        </FormItem>
              ...
              <Form inline class="card-style-form" @submit.native.prevent>
                <FormItem>
                  <Select class="query_more_select" v-model="req.selectName">
                    <Option value="name">{{ "Rule" + $t("name") }}</Option>
...
<script>
import { getpagelistReq, addReq, modifyReq, exportReq } from "@/api/system-manager/rule-info";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: "Rule" + this.$t("name"), key: "name", sortable: "custom" },
  ...
  methods: {
    ...
    editClick() {
      if (this.selectObj) {
        let { id, name, remark, sortCode, enabled } = this.selectObj;
        this.submitData = { id, name, remark, sortCode, enabled };
        this.drawerFlag = true;
        this.isAdd = false;
    ...
    exportClick() {
      ...
      exportReq(obj).then((res) => {
        ...
        const fileName = `${this.$t("rule-info")}${formatDate(
          new Date(),
          "yyyy-MM-dd hh:mm:ss"
        )}.xlsx`; // 自定义文件名
