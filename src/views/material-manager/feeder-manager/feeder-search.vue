  ...
  <div class="page-style">
    ...
    <Modal class-name="page-style-modal" v-model="modalFlag" width="1000" :styles="{ top: '20px' }"
      ...
      <page-custom :total="logSearchReq.total" :totalPage="logSearchReq.totalPage" :pageIndex="logSearchReq.pageIndex"
                   :page-size="logSearchReq.pageIndex" @on-change="pageChangeModal"
      ...
      <Card :bordered="false" dis-hover class="card-style split-right-card">
        <div slot="title">
          <Row>
              ...
              <Poptip v-model="poptipModal" class="poptip-style" placement="right-start" width="400">
                ...
                <div class="poptip-style-content" slot="content">
                  <Form :label-width="80" :label-colon="true" @submit.native.prevent>
                    <FormItem :label="$t('feederState')">
                      <Select v-model="searchReq.data.fidStt" clearable filterable @on-change="selectChange('fidStt')"
                              :placeholder="$t('pleaseSelect') + $t('feederState')">
                        <Option v-for="(item, i) in feederData" :value="item.detailName" :key="i">{{ item.detailName }}</Option>
                      </Select>
                    </FormItem>
                    <FormItem :label="$t('lineWarehouse')">
                      <Select v-model="searchReq.data.lineWarehouseName" clearable filterable
                              @on-change="selectChange('lineWarehouseName')"
                              :placeholder="$t('pleaseSelect') + $t('lineWarehouse')">
                        <Option v-for="(item, i) in lineWarehouseData" :value="item.detailCode" :key="i">{{item.detailName }}</Option>
                      </Select>
                    </FormItem>
                    <FormItem :label="$t('theWarehouse')">
                      <Select v-model="searchReq.data.warehouse" clearable filterable
                              @on-change="selectChange('warehouse')"
                              :placeholder="$t('pleaseSelect') + $t('theWarehouse')">
                        <Option v-for="(item, i) in warehouseData" :value="item.detailCode" :key="i">{{ item.detailName }}</Option>
                      </Select>
                    </FormItem>
                  </Form>
                </div>
            ...
            <i-col span="12">
              <button-custom :btnData="btnData" @on-changeRecord-click="changeRecordClick"
        ...
        <page-custom :total="searchReq.total" :totalPage="searchReq.totalPage" :pageIndex="searchReq.pageIndex"
...
<script>
  import {getpagelistReq, exportReq, getsinglefeederlogReq, exportsinglefeederlogReq} from '@/api/material-manager/feeder-in'
  ...
  import {formatDate, getButtonBoolean, exportFile, formatOperationType} from '@/libs/tools'
  ...
  export default {
    ...
    methods: {
      ...
      async getDataItemData() {
        this.feederData = await this.getDataItemDetailList('FidStt')
        this.lineWarehouseData = await this.getDataItemDetailList('WareHouseLine')
      ...
      // 点击搜索按钮触发
      selectChange(name) {
        if (!this.searchReq.data[name]) this.searchReq.data[name] = ''
        this.pageLoad()
      },
      // 点击搜索按钮触发
      searchClick() {
      ...
      // 工单导出
      exportClick() {
        exportReq({...this.searchReq.data}).then(res => {
