...
<script>
  import {getpagelistReq, addReq, modifyReq, exportReq} from '@/api/basis-info/ebom-avl'
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {title: this.$t('partNO'), key: 'partNO', sortable: 'custom', width: 120, fixed: 'left'},
        ...
        // 验证实体
        ruleValidate: {
          partNO: [{required: true, message: `${this.$t('pleaseEnter')}${this.$t('partNO')}`, trigger: 'blur'}],
          itemPartId: [{required: true, message: `${this.$t('pleaseEnter')}${this.$t('itemPartId')}`, trigger: 'change'}],
          itemAVLId: [{required: true, message: `${this.$t('pleaseEnter')}${this.$t('itemAVLId')}`, trigger: 'change'}],
        },
        btnData: []
      }
    },
    ...
    methods: {
      ...
      pageLoad() {
        ...
        let {id, partNO, itemPartId, processId, itemAVLId, enabled} = this.req
        const obj = {
          orderField: this.req.orderField, // 排序字段
          ascending: this.req.ascending, // 是否升序
          pageSize: this.req.pageSize, // 分页大小
          pageIndex: this.req.pageIndex, // 当前页码
          data: {id, partNO, itemPartId, processId: processId || '', itemAVLId, [this.req.selectName]: this.req.queryName.trim(), enabled}
        };
      ...
      editClick() {
        if (this.selectObj) {
          let {id, partNO, itemPartId, itemAVLId, processId, version, location, qty, unit, remark, sortCode, enabled} = this.selectObj;
      ...
      exportClick() {
        ...
        exportReq({id, partNO, itemPartId, processId: processId || '', itemAVLId,
          ...
          const fileName = `${this.$t('ebom-avl')}${formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss')}.xlsx` // 自定义文件名
