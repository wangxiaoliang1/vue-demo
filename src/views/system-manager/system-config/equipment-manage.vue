      ...
      <Form ref="req" :model="req" :rules="ruleValidate" :label-width="90" :label-colon="true">
        ...
        <FormItem :label="$t('type')" prop="category">
          <RadioGroup v-model="req.category">
            <Radio :label="0" :disabled="!(req.category === 0)">{{ $t("area") }}</Radio>
            <Radio :label="1" :disabled="!(req.category === 1)">{{ $t("broadHeading") }}</Radio>
            <Radio :label="2" :disabled="!(req.category === 2)">{{ $t("equipment") }}</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem :label="$t('parent')" prop="parentId" v-if="req.category != 0">
          <Select
            v-model="req.parentId"
            :clearable="true"
            :placeholder="$t('pleaseSelect') + $t('parent')"
          >
            <Option v-for="(item, i) in parentList" :value="item.id" :key="i">{{
              item.name
            }}</Option>
          </Select>
        </FormItem>
...
import {
  ...
  modifyReq,
  addReq,
} from "@/api/system-manager/equipment-manage";
import {
  formatDate,
  getButtonBoolean,
  errorType,
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        { title: this.$t("code"), key: "enCode" },
        {
          ...
          key: "category",
          render: (h, params) => {
            return h("div", eqpCategory(params.row.category));
          },
        },
  ...
  methods: {
    ...
    // 点击新增按钮触发
    addClick() {
      if (this.selectCategory === 3) return this.$Message.warning(this.$t("addChildNode"));
      ...
      this.req.category = this.selectCategory;
      this.parentList = this.treeList.filter((o) => o.category === this.selectCategory - 1);
    },
    // 新增菜单、按钮
    okClick() {
