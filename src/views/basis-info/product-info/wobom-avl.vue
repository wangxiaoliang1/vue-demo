    ...
    >
      ...
      >
        ...
        <FormItem :label="$t('location')" prop="location">
          <Input v-model="submitData.location" :placeholder="$t('pleaseEnter') + $t('location')" />
        </FormItem>
        <!-- 版本 -->
...
<script>
import { getpagelistReq, addReq, modifyReq, exportReq } from "@/api/basis-info/wobom-avl";

import { formatDate, getButtonBoolean, errorType, renderSpan, exportFile, isLocationAndQtyOk, } from "@/libs/tools";
  ...
  methods: {
    ...
    exportClick() {
      ...
      exportReq(obj).then((res) => {
        ...
        const fileName = `${this.$t("wobom-avl")}${formatDate(
          new Date(),
          "yyyy-MM-dd hh:mm:ss"
        )}.xlsx`; // 自定义文件名
