<template>
  <div class="page-style">
    ...
    >
      ...
      >
        <FormItem :label="$t('lineWarehouse')" prop="warehouseLineId">
          <Select
            v-model="submitData.warehouseLineId"
            clearable
            filterable
            @on-open-change="getWareHouseLineData"
            @on-select="autoFocus = 1"
            :placeholder="$t('pleaseSelect') + $t('lineWarehouse')"
          >
            <Option value="">{{ $t("pleaseSelect") }} </Option>
            <Option v-for="(item, index) in wareHouseLineData" :value="item.detailCode" :key="index"
              >{{ item.detailName }}
            </Option>
          </Select>
        </FormItem>

        <FormItem :label="$t('pdlineId')" prop="lineName">
          <Select
            ...
            @on-select="autoFocus = 1"
            :placeholder="$t('pleaseSelect') + $t('pdlineId')"
          >
            <Option value="">{{ $t("pleaseSelect") }} </Option>
        ...
        <FormItem :label="$t('recipientsWho')" prop="recuser">
          <!--  -->
          <Input
        ...
        <div slot="title">
          <Row type="flex" justify="space-between">
            ...
            <i-col>
              <button-custom
                :btnData="btnData"
                @on-outBound-click="outBoundClick"
...
export default {
  ...
  methods: {
    getWareHouseLineData(e) {
      if (e) {
        const obj = {
          itemCode: "WareHouseLine",
          enabled: 1,
        };
        getlistReq(obj).then((res) => {
          if (res.code === 200) {
            this.wareHouseLineData = res.result || [];
          }
        });
      }
    },
    ...
    okClick() {
      this.$refs.req.validate((validate) => {
        if (validate) {
          if (!this.submitData.warehouseLineId && !this.submitData.lineName) {
            playSound(this.$config.tipsVoice.NgTips);
            this.errData.unshift({
              isSuccess: false,
              text: "线边仓/线别 必须选一个",
            });
          } else if (this.submitData.warehouseLineId && this.submitData.lineName) {
            playSound(this.$config.tipsVoice.NgTips);
            this.errData.unshift({
              isSuccess: false,
              text: "线边仓/线别 只能选一个",
            ...
            let detailName = this.wareHouseLineData.filter((o) => {
              return o.detailCode === this.submitData.warehouseLineId;
            });
            let obj = {
              floor: this.req.floor,
              warehouseLineId: this.submitData.warehouseLineId || "",
              warehouseLineName: detailName.length > 0 ? detailName[0].detailName : "",
              feeder: this.submitData.feeder.trim(),
              recuser: this.submitData.recuser.trim(),
              lineName: this.submitData.lineName || "",
              enabled: 1,
            };
            setfeederoutReq(obj).then((res) => {
              if (res.code === 200) {
                playSound(this.$config.tipsVoice.OkTips);
                this.$Message.success(`${this.$t("outBound")}${this.$t("success")}`);
                this.errData.unshift({
                  isSuccess: true,
                  text: `${this.$t("outBound")}${this.$t("success")}`,
                });
              } else {
                playSound(this.$config.tipsVoice.NgTips);
                this.$Message.error(
                  `${this.$t("outBound")}${this.$t("fail")},${errorType(this, res.errorCode)}`
                );
                this.errData.unshift({
                  isSuccess: false,
                  text: `${this.$t("outBound")}${this.$t("fail")},${errorType(
                    this,
                    res.errorCode
                  )}`,
                });
    ...
    outBoundClick() {
      ...
      this.autoFocus = 2;
      this.drawerTitle = this.$t("outBound");
    },
    //获取变更记录
    changeRecordClick() {
      if (this.selectObj) {
