    ...
    >
      ...
      >
        ...
        <FormItem :label="$t('type')" prop="category">
          <RadioGroup v-model="submitData.category">
            <Radio :label="0">{{ $t("provider") }}</Radio>
            <Radio :label="1">{{ $t("manufacturer") }}</Radio>
            <Radio :label="2">{{ $t("customer") }}</Radio>
          </RadioGroup>
        </FormItem>
            ...
            <i-col span="12">
              <Form inline class="card-style-form" @submit.native.prevent>
                ...
                <FormItem style="width: 100px;">
                  ...
                  >
                    <Option :value="0">{{ $t("provider") }}</Option>
                    <Option :value="1">{{ $t("manufacturer") }}</Option>
...
<script>
import { getpagelistReq, addReq, modifyReq, exportReq } from "@/api/system-manager/customer-vendor";
import {
  formatDate,
  getButtonBoolean,
  errorType,
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: this.$t("enCode"), key: "enCode" },
        { title: this.$t("remark"), key: "remark" },
        {
          title: this.$t("type"),
          key: "category",
          render: (h, params) => {
            return h("div", customerVendorType(params.row.category));
          },
        },
        { title: this.$t("addr"), key: "addr" },
        { title: this.$t("tel"), key: "tel" },
        {
  ...
  methods: {
    ...
    editClick() {
      if (this.selectObj) {
        let { id, name, enCode, category, addr, tel, remark, enabled } = this.selectObj;
        this.submitData = { id, name, enCode, category, addr, tel, remark, enabled };
        this.drawerFlag = true;
        this.isAdd = false;
    ...
    exportClick() {
      ...
      exportReq(obj).then((res) => {
        ...
        const fileName = `${this.$t("customer-vendor")}${formatDate(
          new Date(),
          "yyyy-MM-dd hh:mm:ss"
        )}.xlsx`; // 自定义文件名
