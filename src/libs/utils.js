 ...
 * @param {String} route 当前关闭的标签的route
 */
...
export const initNode = (data) => {
  ...
  for (let item of data) {
    ...
    if (hasChild(item)) initNodeChildren(menu.children, item.children)
    menuList.push(menu)
  }
  return menuList
}
 ...
 * @node 接口返回子节点数组
 * */
const initNodeChildren = (list, node) => {
  for (let item of node) {
    let menuNode = {
      ...
      component: hasChild(item) ? () => import('@/components/parent-view') : () => import(`@/views/${item.component}`),
      children: []
    }
    if (hasChild(item)) initNodeChildren(menuNode.children, item.children)
    list.push(menuNode)
  }
}
